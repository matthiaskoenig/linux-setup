...
cmake -DWITH_BZIP2=OFF -DWITH_ZLIB=OFF -DLIBSBML_USE_LEGACY_MATH=ON -DENABLE_COMP=ON -DENABLE_FBC=ON -DENABLE_GROUPS=ON -DENABLE_LAYOUT=ON -ENABLE_GROUPS -DENABLE_QUAL=ON -DWITH_EXAMPLES=ON -DWITH_R=ON -DWITH_PYTHON=ON -DWITH_CREATE_PYTHON_SOURCE=ON ${SVN_DIR}/$SBMLCODE/libsbml
